    ...
    try:
        while True:
            ...
            if unreachable_hosts:
                print(f"[WARN] unreachable hosts: {', '.join(unreachable_hosts)}")
            if free_hosts:
                for host in free_hosts:
                    ...
                    print(f"[{time.strftime('%H:%M:%S')}] Launching on {host}: {job_cmd[:120]}{'...' if len(job_cmd)>120 else ''}")
                    res = submit_to_host(host, job_cmd, payload_local_path=payload)
                    if not res.get("ok"):
                        print(f"  Failed to start on {host}: {res.get('err')}")
                        if not res.get('err')=="pidfile present but process not running":
                            q = load_queue()
                            q.insert(0, job_item)
                            save_queue(q)
                        else:
                            print(f"    Job Failed on {host}; but had pid file. Job maybe had error.")
                    else:
                        print(f"  Started {res.get('tag')} pid={res.get('pid')} payload={res.get('payload', '-')}")
                        tag = res.get("tag")
